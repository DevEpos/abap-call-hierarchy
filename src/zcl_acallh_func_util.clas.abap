...
CLASS zcl_acallh_func_util DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
    ...
    CLASS-METHODS:
      ...
      get_function_module_info
        IMPORTING
          function_module TYPE tfdir-funcname
        RETURNING
          VALUE(result)   TYPE zif_acallh_ty_global=>ty_function_info
        RAISING
          zcx_acallh_not_exists,
...
CLASS zcl_acallh_func_util IMPLEMENTATION.
  ...
  METHOD get_function_module_info.
    ...
    IF sy-subrc <> 0.
      CLEAR result.
      RAISE EXCEPTION TYPE zcx_acallh_not_exists
        EXPORTING
