"! <p class="shorttext synchronized">Where-Used-List Service</p>
CLASS zcl_acallh_where_used_srv DEFINITION
  ...
  PRIVATE SECTION.
    ...
    TYPES ty_referencing_object_types TYPE STANDARD TABLE OF ty_referencing_object_type WITH NON-UNIQUE DEFAULT KEY.

    TYPES: BEGIN OF ty_ris_referenced_object,
    ...
    DATA abap_elem_fac TYPE REF TO zif_acallh_abap_element_fac.
    DATA current_element TYPE REF TO zif_acallh_abap_element.
    DATA current_element_info TYPE zif_acallh_ty_global=>ty_abap_element.
    DATA incomplete_types_range TYPE RANGE OF string.
    DATA incomplete_elems TYPE zif_acallh_ty_global=>ty_abap_elements.
    DATA result_elems_raw TYPE zif_acallh_ty_global=>ty_abap_elements.

    METHODS get_referenced_obj_from_data
      RETURNING
    ...
    METHODS get_individual_request
      IMPORTING
        referenced_object TYPE ty_ris_referenced_object
      RETURNING
    ...
    METHODS extract_attr_from_ris_result
      IMPORTING
        generic_ris_result TYPE sris
        referenced_object  TYPE ty_ris_referenced_object
      RETURNING
    ...
    METHODS create_result_elements
      RETURNING
        VALUE(result) TYPE zif_acallh_abap_element=>ty_ref_tab.

    METHODS complete_elements.
ENDCLASS.
...
CLASS zcl_acallh_where_used_srv IMPLEMENTATION.
  ...
  METHOD get_referenced_obj_from_data.
    ...
    result = VALUE #( full_name    = COND #( WHEN current_element_info-alias_full_name IS NOT INITIAL
                                             THEN current_element_info-alias_full_name
                                             ELSE current_element_info-full_name )
                      main_program = current_element_info-main_program
                      object       = VALUE #( name           = current_element_info-object_name
                                              enclosing_name = current_element_info-encl_object_name
                                              type           = CORRESPONDING #( current_element_info-type ) )
                      scope_object = VALUE #( name           = current_element_info-scope_object-object_name
    ...
    LOOP AT raw_object_types INTO DATA(raw_object_type) WHERE    trobjtype = zif_acallh_c_tadir_type=>class
                                                              OR trobjtype = zif_acallh_c_tadir_type=>interface
                                                              OR trobjtype = zif_acallh_c_tadir_type=>program.
  ...
  METHOD get_results.
    ...
    DATA individual_results TYPE ris_t_results.
    ...
    LOOP AT referenced_object-referencing_object_types INTO referencing_object_type.
      ...
      TRY.
          referencing_object_type-data_model->execute_where_used(
            EXPORTING is_parameter_where_used = individual_request
        ...
        CATCH cx_ris_exception INTO DATA(ris_exception). " TODO: variable is assigned but never used (ABAP cleaner)
      ENDTRY.
  ...
  METHOD get_main_program.
    " TODO: parameter REFERENCED_OBJECT is only used in commented-out code (ABAP cleaner)
...
*    ENDIF.
  ...
  METHOD convert_to_abap_elem.
    result = extract_attr_from_ris_result( generic_ris_result = generic_ris_result
                                           referenced_object  = referenced_object ).

    IF result IS INITIAL.
      RAISE EXCEPTION TYPE zcx_acallh_exception
        EXPORTING text = 'No valid result'.
    ELSEIF     result-object_name      = current_element_info-object_name
           AND result-encl_object_name = current_element_info-encl_object_name.
      RAISE EXCEPTION TYPE zcx_acallh_exception
        EXPORTING text = 'No valid result'.
    ENDIF.
    ...
    IF     result-full_name IS NOT INITIAL
       AND result-tag        = cl_abap_compiler=>tag_method.
  ...
  METHOD extract_attr_from_ris_result.
    ...
    IF     result-include+30(1) = seop_inctype_class
       AND (    result-include+31(1) = seop_inccode_public
             OR result-include+31(1) = seop_inccode_protected
             OR result-include+31(1) = seop_inccode_private ).
      CLEAR result.
      RETURN.
    ENDIF.
    ...
    ELSEIF generic_ris_result-object_cls = 'P'.
      ...
      CALL FUNCTION 'RS_PROGNAME_SPLIT'
        EXPORTING  progname_with_namespace     = result-include
      ...
      IF sy-subrc = 0.
        IF is_function = abap_true.
          ...
          CALL FUNCTION 'FUNCTION_INCLUDE_INFO'
            CHANGING   funcname = function_name
  ...
  METHOD set_full_name.
    " TODO: parameter GENERIC_RIS_RESULT is never used (ABAP cleaner)
    " TODO: parameter REFERENCED_OBJECT is never used (ABAP cleaner)
  ...
  METHOD complete_elements.
    ...
    " Calls from local class include does not return concrete line matches and need to be evaluated with
    " the compiler
    LOOP AT incomplete_elems REFERENCE INTO DATA(incomplete_item) GROUP BY incomplete_item->main_program.
      CHECK incomplete_item->main_program IS NOT INITIAL.

      DATA(compiler) = zcl_acallh_abap_compiler=>get( main_prog = incomplete_item->main_program ).

      compiler->set_include( include = incomplete_item->include ).

      " TODO: variable is assigned but never used (ABAP cleaner)
      DATA(called_method_refs) = compiler->get_refs_by_fullname( full_name = incomplete_item->full_name
                                                                 grade     = cl_abap_compiler=>grade_direct ).

      " TODO: collect found references
