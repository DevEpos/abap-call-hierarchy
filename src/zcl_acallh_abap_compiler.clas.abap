...
CLASS zcl_acallh_abap_compiler DEFINITION
  PUBLIC FINAL
  CREATE PRIVATE.

  PUBLIC SECTION.
    INTERFACES zif_acallh_abap_compiler.
...
CLASS zcl_acallh_abap_compiler IMPLEMENTATION.
  METHOD get.
    ...
    ELSE.
      result = NEW zcl_acallh_abap_compiler( main_prog = main_prog
                                             compiler  = compiler ).
    ENDIF.
  ENDMETHOD.
  ...
  METHOD zif_acallh_abap_compiler~get_check_errors.
    abap_compiler->get_check_infos( IMPORTING p_errors = result ).
  ENDMETHOD.
  ...
  METHOD zif_acallh_abap_compiler~get_refs_by_fullname.
  ...
  METHOD zif_acallh_abap_compiler~get_refs_by_fullnames.
  ...
  METHOD zif_acallh_abap_compiler~get_symbol_entry.
    result = abap_compiler->get_symbol_entry( p_full_name = full_name ).
  ENDMETHOD.

  METHOD zif_acallh_abap_compiler~get_refs_in_range.
  ...
  METHOD zif_acallh_abap_compiler~get_alias_fullname.
    abap_compiler->get_alias_full_name( EXPORTING  p_full_name                = full_name
                                        IMPORTING  p_alias_full_name          = result
...
*                p_abort    =
  ...
  METHOD zif_acallh_abap_compiler~set_include.
    abap_compiler->set_includes( p_include = include ).
  ENDMETHOD.
ENDCLASS.
