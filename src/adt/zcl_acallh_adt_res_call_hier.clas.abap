"! <p class="shorttext synchronized">Resource for ABAP Call Hierarchy</p>
CLASS zcl_acallh_adt_res_call_hier DEFINITION
  ...
  PRIVATE SECTION.
    ...
    METHODS create_callers_result
      IMPORTING
        root_element  TYPE REF TO zif_acallh_abap_element
      RETURNING
        VALUE(result) TYPE zif_acallh_ty_adt=>ty_call_hierarchy_result.

    METHODS get_path_type
      IMPORTING
...
CLASS zcl_acallh_adt_res_call_hier IMPLEMENTATION.
  METHOD get.
    ...
    DATA hierarchy_settings TYPE zif_acallh_ty_global=>ty_hierarchy_api_settings.

    DATA(path) = zcl_acallh_adt_request_util=>get_query_parameter(
                     param_name = zif_acallh_c_adt=>c_call_hierarchy_params-path
                     mandatory  = abap_true
                     request    = request ).

    DATA(mode) = zcl_acallh_adt_request_util=>get_query_parameter(
                     param_name = zif_acallh_c_adt=>c_call_hierarchy_params-mode
                     mandatory  = abap_true
                     request    = request ).
    ...
    hierarchy_settings-use_first_intf_impl = zcl_acallh_adt_request_util=>get_boolean_query_parameter(
        param_name = zif_acallh_c_adt=>c_call_hierarchy_params-auto_resolve_intf_method
        request    = request ).
    ...
    TRY.
      ...
      CATCH zcx_acallh_exception INTO DATA(error).
        RAISE EXCEPTION TYPE zcx_acallh_adt_rest
          EXPORTING text     = |Call Hierarchy failed: { error->get_text( ) }|
                    previous = error.
    ENDTRY.

    IF root_element IS NOT INITIAL.
      IF mode = zif_acallh_c_adt=>c_hierarchy_mode-callees.
        hierarchy_result = create_callees_result( root_element       = root_element
                                                  hierarchy_settings = hierarchy_settings ).
      ELSE.
        hierarchy_result = create_callers_result( root_element = root_element ).
      ENDIF.

      response->set_body_data( content_handler = zcl_acallh_ch_factory=>create_call_hier_result_ch( )
  ...
  METHOD get_path_type.
    result = zcl_acallh_adt_request_util=>get_query_parameter(
                 param_name = zif_acallh_c_adt=>c_call_hierarchy_params-path_type
                 mandatory  = abap_true
                 request    = request ).

    IF     result <> zif_acallh_c_adt=>c_path_types-full_name
       AND result <> zif_acallh_c_adt=>c_path_types-uri.
      RAISE EXCEPTION TYPE zcx_acallh_adt_rest
        EXPORTING text = |Value { result } of query parameter 'pathType' is not valid | &&
                         |[{ zif_acallh_c_adt=>c_path_types-full_name },{ zif_acallh_c_adt=>c_path_types-uri }]|.
    ENDIF.
  ENDMETHOD.

  METHOD create_callees_result.
    result = create_result( root_element = root_element
  ...
  METHOD create_result.
    ...
    result = VALUE #( origin_type              = root_element->element_info-adt_type
  ...
  METHOD get_root_element.
    IF path_type = zif_acallh_c_adt=>c_path_types-uri.
