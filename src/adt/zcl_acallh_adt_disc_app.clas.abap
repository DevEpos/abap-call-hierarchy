"! <p class="shorttext synchronized">Router for ABAP Call Hierarchy</p>
CLASS zcl_acallh_adt_disc_app DEFINITION
  ...
  PRIVATE SECTION.
    CONSTANTS c_static_uri TYPE string VALUE '/devepos/adt/aht'.
    CONSTANTS c_call_hierarchy_uri TYPE string VALUE '/callhierarchy'.
    CONSTANTS c_root_scheme TYPE string VALUE 'http://www.devepos.com/adt/aht'.
    CONSTANTS c_root_rel_scheme TYPE string VALUE 'http://www.devepos.com/adt/relations/aht'.
    ...
    METHODS register_call_hierarchy
      IMPORTING
...
CLASS zcl_acallh_adt_disc_app IMPLEMENTATION.
  ...
  METHOD register_call_hierarchy.
    ...
    DATA(template) = |{ c_call_hierarchy_uri }| &&
                     |\{?{ zif_acallh_c_adt=>c_call_hierarchy_params-path }*\}| &&
                     |\{&{ zif_acallh_c_adt=>c_call_hierarchy_params-path_type }*\}| &&
                     |\{&{ zif_acallh_c_adt=>c_call_hierarchy_params-mode }*\}| &&
                     |\{&{ zif_acallh_c_adt=>c_call_hierarchy_params-auto_resolve_intf_method }*\}| &&
                     |\{&{ zif_acallh_c_adt=>c_call_hierarchy_params-intf_method_implementation }*\}|.

    call_hierarchy_coll->register_disc_res_w_template( relation      = c_root_rel_scheme && c_call_hierarchy_uri
                                                       template      = template
                                                       handler_class = c_handlers-call_hierarchy ).
  ENDMETHOD.
ENDCLASS.
