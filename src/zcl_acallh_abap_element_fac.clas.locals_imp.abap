...
CLASS lcl_intfm_src_pos_read_fac IMPLEMENTATION.
  ...
  METHOD get_fugrnames_from_uri.
    ...
    main = zcl_acallh_func_util=>get_progname_for_group( CONV #( group ) ).
    ...
    IF type_name = 'fmodules'.
      include = zcl_acallh_func_util=>get_function_include_by_fname( CONV #( include ) ).
    ENDIF.
  ENDMETHOD.
...
CLASS lcl_intfm_src_pos_reader_base IMPLEMENTATION.
  ...
  METHOD resolve_target.
    ...
    DATA(full_name_info) = zcl_acallh_fullname_util=>get_info_obj( full_name_prefix ).
  ...
  METHOD resolve_by_navigation.
    TRY.
        DATA(target_uri) = zcl_acallh_adt_nav_util=>navigate_by_uri(
          uri    = orig_uri
          source = source_code ).
    ...
    DATA(reader) = lcl_intfm_src_pos_read_fac=>create_reader_by_uri(
      ...
      full_name       = full_name
      source_position = zcl_acallh_adt_uri_util=>get_uri_source_start_pos( target_uri ) ).
