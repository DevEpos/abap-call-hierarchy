...
CLASS zcl_acallh_comp_unit_factory DEFINITION
  PUBLIC
  FINAL
  CREATE PRIVATE.

  PUBLIC SECTION.
    INTERFACES zif_acallh_comp_unit_factory.

    CLASS-METHODS:
      "! <p class="shorttext synchronized" lang="en">Retrieves factory instance</p>
      get_instance
        RETURNING
          VALUE(result) TYPE REF TO zif_acallh_comp_unit_factory.
  PROTECTED SECTION.
  PRIVATE SECTION.
    CLASS-DATA:
      instance TYPE REF TO zif_acallh_comp_unit_factory,
      parser   TYPE REF TO zcl_acallh_abap_parser.
    ...
    METHODS:
      ...
      resolve_main_prog
        IMPORTING
          unit_data TYPE REF TO zif_acallh_ty_global=>ty_compilation_unit,
...
CLASS zcl_acallh_comp_unit_factory IMPLEMENTATION.
  ...
  METHOD get_instance.
    IF instance IS INITIAL.
      instance = NEW zcl_acallh_comp_unit_factory( ).
    ENDIF.
  ...
  METHOD zif_acallh_comp_unit_factory~create_comp_unit.
    ...
    result = NEW zcl_acallh_compilation_unit(
      data              = l_unit_data
      hierarchy_service = zcl_acallh_call_hierarchy=>get_call_hierarchy_srv( ) ).
  ENDMETHOD.


  METHOD zif_acallh_comp_unit_factory~create_comp_unit_from_ext.
    IF data_request-orig_request-legacy_type NOT IN relevant_legacy_types.
      RAISE EXCEPTION TYPE zcx_acallh_exception.
    ENDIF.

    DATA(unit_data) = CORRESPONDING zif_acallh_ty_global=>ty_compilation_unit( data_request-orig_request ).

    resolve_main_prog( REF #( unit_data ) ).

    IF unit_data-main_program IS INITIAL.
      RAISE EXCEPTION TYPE zcx_acallh_exception.
    ENDIF.

    unit_data-tag = zcl_acallh_fullname_util=>get_info_obj( unit_data-full_name )->get_abap_fullname_tag( ).
    ...
    DATA(comp_unit) = NEW zcl_acallh_compilation_unit(
      data              = unit_data
      hierarchy_service = zcl_acallh_call_hierarchy=>get_call_hierarchy_srv( ) ).
    ...
    ELSE.
      ...
      IF sy-subrc = 0.
        ...
        ELSE.
          ...
          DATA(name_parts) = zcl_acallh_fullname_util=>get_parts( unit_data->full_name ).
  ...
  METHOD determine_source_info.
    ...
    DATA(compiler) = zcl_acallh_abap_compiler=>get( main_prog = unit_data->main_program ).
    ...
    IF source_info IS INITIAL AND unit_data->tag = cl_abap_compiler=>tag_method.
      IF unit_data->method_props-is_alias = abap_true.
        ...
        LOOP AT refs ASSIGNING FIELD-SYMBOL(<ref>) WHERE full_name = unit_data->full_name.
          ...
          ENDIF.
        ENDLOOP.
      ELSEIF unit_data->encl_object_type = zif_acallh_c_tadir_type=>interface.
